...
export const levelShiftSchema = z
  .object({
    level: z.enum([Level.N1, Level.N2, Level.ESPC]),
    shift: z.enum([Shift.T1, Shift.T2, Shift.T3, Shift.TC]),
  })
  .refine(
    (data) => {
      if (data.level === Level.N2 && data.shift === Shift.T3) return false;
      if (data.shift === Shift.T3 && data.level === Level.ESPC) return false;
      if (data.level === Level.ESPC && data.shift !== Shift.TC) return false;
      if (data.shift === Shift.TC && data.level !== Level.ESPC) return false;
      return true;
    },
    {
      message:
        "N2 só pode estar nos turnos T1 ou T2. T3 aceita apenas N1. ESPC deve usar turno TC.",
      path: ["shift"],
    }
  );
