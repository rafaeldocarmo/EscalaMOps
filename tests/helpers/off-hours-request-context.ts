...
export async function createOffHoursRequestTestContext(
  options: { dateKey?: string; balanceHours?: number } = {},
  ...
  const team = await prisma.team.create({ data: { name: `mops-bh-${suffix}`, isDefault: false } });
  ...
  await prisma.bankHourBalance.create({ data: { memberId: member.id, balanceHours: options.balanceHours ?? 40 } });

  const schedule = await prisma.schedule.create({
    data: { teamId: team.id, year, month, status: ScheduleStatus.OPEN },
  });

  return { teamId: team.id, memberId: member.id, scheduleId: schedule.id, dateKey, workDateUtc };
}
...
export async function createOffAssignmentOnDate(ctx: OffHoursRequestTestContext): Promise<void> {
  await prisma.scheduleAssignment.create({
    data: { scheduleId: ctx.scheduleId, memberId: ctx.memberId, date: ctx.workDateUtc, status: AssignmentStatus.OFF },
  });
}

export async function createPendingOffHoursRequestSameDay(ctx: OffHoursRequestTestContext, hours: number): Promise<void> {
...
export async function createMemberTeamWithoutSchedule(): Promise<{ teamId: string; memberId: string }> {
  const suffix = randomBytes(4).toString("hex");
  const team = await prisma.team.create({ data: { name: `mops-bh-ns-${suffix}`, isDefault: false } });
  const { levelIds, shiftIds } = await ensureCatalogForTeam(team.id);
  const member = await prisma.teamMember.create({
    data: {
      teamId: team.id,
